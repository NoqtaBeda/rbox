...
rbox provides functionality to generate compile-time fixed map structures whose keys are of either integral, enum or string type, supporting auto selection among numerous data structures and manual fine-tuning to the underlying data structure with various configuration parameters.

Multi-maps are not supported. A compilation error will occur if duplicate keys exist in the input entries.

Fixed map serves as the base of multiple rbox components, including but not limited to:

- [Enum integer-to-string and string-to-integer conversion](./enum.md#enum-operations);
- [Lookup table](./lookup.md).

> Even in the LLM era, rbox has its unique advantage that an efficient data structure can be applied automatically with your code kept clean. You don't have to worry that (1) you forget to instruct the agent to update all related components; (2) the agent generates flawed code for several items which is hard to detect by human review (especially when the number of items or related components is large); (3) the agent either fails to pick some good data structure, or generates a huge block of boilerplate code which harms code readability.
...
The underlying data structure of integral-key fixed map can be one of the following, selected by rbox automatically according to the input data:

1. **Fully-dense, O(1)**: Applied only if $k_{\text{max}} - k_{\text{min}} = n - 1$ , i.e. _all_ the keys are contiguous. The underlying data structure is an array of values sorted by their corresponding keys, plus `min_key` and `max_key` fields denoting the key range;
2. **Dense, O(1)**: Applied only if $k_{\text{max}} - k_{\text{min}} + 1 \le n/\alpha$ where $\alpha$ is the minimum load factor (default value is 0.5). The underlying data structure is an array of length $k_{\text{max}} - k_{\text{min}} + 1$ indexed by $\text{key} - k_{\text{min}}$, plus `min_key` and `max_key` fields denoting the key range. How _holes_ (positions with no corresponding input entry) are represented depends on the value type `V`:
    - If `V` is one of scalar types (arithmetic, pointers, etc.), `meta_span` or `meta_basic_string_view`, and no input key equals to `V{}`, then each array slot stores the `value` alone and holes are simply `V{}`;
    - Otherwise, each array slot stores a `(value, is_valid)` pair, where `is_valid=false` marks a hole;
...
4. **General, O(log n)**: The underlying data structure is decomposed into 3 parts: the middle dense (or fully-dense) part whose key falls in range $[k_{\text{min}}', k_{\text{max}}']$, the left sparse (or empty) part whose key $< k_{\text{min}}'$, and the right sparse (or empty) part whose key $> k_{\text{max}}'$.
...
2. Dense (with is_valid flag)
kv-pairs: {3: "A3", 5: "B5", 6: "", 8: "D8"}
min_key = 3
max_key = 10
array:
+-------------------------------------------------------------------------------------------------+
|  value="A3"   |    value=""    |  value="B5"   |    value=""   |    value=""    |  value="D8"   |
...
+-----------------------------------------------------------------------------------------+
pseudo-code:
if key < min_key or key > max_key or array[key - min_key] == V{}:
    return nullopt
return array[key - min_key]
...
Let $n$ be the number of input entries, $L$ be the maximum length of input keys. The underlying data structure of string-key fixed map can be one of the following, selected by rbox automatically according to the input data:
...
Defined in header `<rbox/fixed_map/integral_key.hpp>`.
...
The generated fixed map supports the following operations (`using value_type = to_static_storage_result_t<InputValueType>`, _signedness-safe and narrowing-safe_ key comparison applied):
...
- `already_sorted` (default: `false`): Whether input key-value-pair range is already sorted. This option helps improve compile-time performance by skipping sorting if it is ensured that kv-pairs are sorted already. UB or wrong result may occur if this flag is set to true but the input range is not actually sorted.
...
constexpr auto map1 = RBOX_INTEGRAL_KEY_FIXED_MAP(map1_entries());
...
Defined in header `<rbox/fixed_map/string_key.hpp>`.
...
The function returns a reflected constant of the fixed map, which can be extracted via `[: V :]`. Typically, the macro `RBOX_STRING_KEY_FIXED_MAP` can be used to extract the constant fixed map object.

The generated fixed map supports the following operations (`using value_type = to_static_storage_result_t<InputValueType>`):
...
- `max_n_hash_probing_attempts` (default: `3`): Maximum number of hash probing attempts to find a suitable slot for each input entry during hash table construction. See section ["Candidate Data Structures"](#candidate-data-structures) above for details.
...
constexpr auto map = RBOX_STRING_KEY_FIXED_MAP(map_entries());
...
constexpr auto ci_map =
    RBOX_STRING_KEY_FIXED_MAP(ci_map_entries(), {.ascii_case_insensitive = true});
