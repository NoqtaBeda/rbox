...
rbox provides functionality for serialization. Currently the following are supported:
* Serialize to JSON
* Debug dump - serialize input to JSON-style debug string.

## Components

### Serialize to JSON

Defined in header `<rbox/serializer/serialize_to_json.hpp>`.
...
template <serialize_options Options = {}, class CharT, class Allocator, serializable T>
constexpr bool serialize_to_json(
...
template <class CharT = char, serialize_options Options = {}, serializable T>
constexpr auto serialize_to_json(
...
Enumeration names containing non-ASCII characters (e.g., Chinese, Japanese, emoji) are fully supported. The builder automatically chooses between ASCII-only fast paths (which call `append_c_string_unsafe()` internally) and UTF conversion paths (which call `append_utf_string_unsafe` internally) based on compile-time analysis of the enum's identifier names.
...
auto b = rbox::serialize_to_json(true);                  // "true"
auto s = rbox::serialize_to_json(std::string{"hello"});  // "\"hello\""

// With options
auto c =
    rbox::serialize_to_json<char, rbox::serialize_options{.char_to_string = true}>('A');  // "\"A\""
auto e = rbox::serialize_to_json<rbox::serialize_options{.enum_to_string = true}>(
    color_t::red);  // "\"red\""
...
person_t p{"Alice", 30};
auto json = rbox::serialize_to_json(p, 2, ' ');
// {
//   "name": "Alice",
//   "age": 30
// }
...
auto nested = rbox::serialize_to_json(data);
// [{"a": 1, "b": 2}]

// Using builder
rbox::u8string_builder builder;
rbox::serialize_to_json(builder, p);
auto result = builder.str();
```

### Dump to JSON-style Format

Defined in header `<rbox/serializer/dump_to_json_like.hpp>`.
...
Unlike `serialize_to_json`, overloads (3) and (4) always return `std::basic_string<CharT>` (string overloads) — they never halt on invalid input. Invalid values are also dumped as human-readable hint message.
...
auto result = rbox::dump_to_json_like<char16_t>(std::u8string{u8"你好"});
// Result: u"\"你好\""
```
